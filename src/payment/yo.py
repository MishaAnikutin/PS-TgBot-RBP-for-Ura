...
from ..secrets.config import PAYMENT_TOKEN, CLIENT_ID, CLIENT_SECRET, REDIRECT_URL, CUSTOMER_CODE
from .models import (
        create_model,
        CreatePaymentModel,
        PaymentInfoModel,
...
class paymentFactory(BasePayment):
    ...
    async def create_payment(cls, value: int) -> CreatePaymentModel:
        ...
        Обязательно используем JWT токен для оплаты, он работает бессрочно
        """
